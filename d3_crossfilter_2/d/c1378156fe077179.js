...
html`<code>css</code> <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.3.0/css/bootstrap.min.css" integrity="sha384-PDle/QlgIONtM1aqA2Qemk5gPOE7wFq8+Em+G/hmo5Iq0CCmYZLv3fVRDJ4MMwEA" crossorigin="anonymous">`
  ...
  main.variable(observer("buildvis")).define("buildvis", ["md","container","dc","dateDimension","d3","magnitudeDimension","magnitudeGroup","depthDimension","depthGroup","dataset","timeDimension","timeGroup"], function(md,container,dc,dateDimension,d3,magnitudeDimension,magnitudeGroup,depthDimension,depthGroup,dataset,timeDimension,timeGroup)
{
  let view = md`${container()}`
  
  let dataTable = dc.dataTable(view.querySelector("#dc-table-graph"), "earthquakes")
  dataTable
    .width(960)
    .height(800)
  ...
  let depthChart = dc.barChart(view.querySelector("#depth-chart"), "earthquakes")
  depthChart
    .width(480)
    .height(150)
    .gap(1)
  ...
  let timeChart = dc.lineChart(view.querySelector("#time-chart"), "earthquakes")
  timeChart
    .width(960)
    .height(150)
    .elasticY(true)
...
d3.csv("https://gist.githubusercontent.com/emanueles/65a308ffa630689c11a031252998ef8d/raw/a004c770786229d54264406118ae21ba7e4c51a8/earthquakes.csv")
  ...
  main.variable(observer("eq")).define("eq", ["crossfilter","dataset"], function(crossfilter,dataset){return(
crossfilter(dataset)
)});
  main.variable(observer("dateDimension")).define("dateDimension", ["eq"], function(eq){return(
eq.dimension(d => d.dtg)
)});
  main.variable(observer()).define(["dateDimension"], function(dateDimension){return(
dateDimension.top(10)
)});
  main.variable(observer("magnitudeDimension")).define("magnitudeDimension", ["eq"], function(eq){return(
eq.dimension(d => d.magnitude)
)});
...
magnitudeDimension.group()
  ...
  main.variable(observer("depthDimension")).define("depthDimension", ["eq"], function(eq){return(
eq.dimension(d => d.depth)
)});
  main.variable(observer("depthGroup")).define("depthGroup", ["depthDimension"], function(depthDimension){return(
depthDimension.group()
)});
  main.variable(observer("timeDimension")).define("timeDimension", ["eq","d3"], function(eq,d3){return(
eq.dimension(d => d3.timeHour(d.dtg))
)});
