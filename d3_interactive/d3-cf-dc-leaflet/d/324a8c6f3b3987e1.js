...
idDimension.group()
  ...
  main.variable(observer("circles")).define("circles", ["d3","dataset","L","circlesLayer"], function(d3,dataset,L,circlesLayer)
{
  let markers = d3.map()
  ...
  main.variable(observer("layerList")).define("layerList", ["idGrouping","circles","L","map"], function(idGrouping,circles,L,map)
{
  let layers = []
  for (let tId of idGrouping.all()) {
    layers.push(circles.get(tId.key))
  }
...
function updateMarkers(){
  ...
  main.variable(observer("updateFilters")).define("updateFilters", ["d3","layerList","idDimension","dc"], function(d3,layerList,idDimension,dc){return(
function updateFilters(e) {
  let markersToDisplay = d3.map();
  layerList[0].eachLayer(layer => {
    markersToDisplay.set(layer.publicid, e.target.getBounds().contains(layer.getLatLng()))
  })
  idDimension.filterFunction(function(d) {
    return markersToDisplay.get(d)
  });
  dc.redrawAll();
}
)});
  ...
  main.variable(observer("map")).define("map", ["view","L"], function(view,L)
{
  ...
  let mapClone = mapContainer.cloneNode(true)
  let mapParent = mapContainer.parentNode
  mapClone.setAttribute('id', 'mapid')
  mapClone.classList.add('map-container')
  mapParent.insertBefore(mapClone, mapContainer.nextSibling); 
  mapParent.removeChild(mapContainer)
  let mapInstance = L.map(mapClone).setView([-41.05,172.93], 5)
...
<main role="main" class="container">
    ...
    <div class='row'>
        <div id="mapid" class="col-6 map-container">
        </div>
        <div class="col-6">
