// https://observablehq.com/@samirbraga/d3-com-crossfilter-e-dc-js@531
...
d3
.csv("https://gist.githubusercontent.com/emanueles/d8df8d875edda71aa2e2365fae2ce225/raw/1e949d3da02ed6caa21fe3a7a12a4e5a611a4bab/stocks.csv")
.then(data => {
  ...
  data.forEach((d, i) => {
       ...
       d.googlePercent = (parseFloat(d.google) - data[0].google) / data[0].google
       d.facebookPercent = (parseFloat(d.facebook) - data[0].facebook) / data[0].facebook
  })
  return data
})
)});
...
md`## Exercício 1`
  ...
  main.variable(observer("googleByDayPercentGroup")).define("googleByDayPercentGroup", ["dateDim"], function(dateDim){return(
dateDim.group().reduceSum(d => d.googlePercent * 100)
)});
  main.variable(observer("fbByDayPercentGroup")).define("fbByDayPercentGroup", ["dateDim"], function(dateDim){return(
dateDim.group().reduceSum(d => d.facebookPercent * 100)
)});
  main.variable(observer("acoes")).define("acoes", ["md","container","dc","d3","dateDim","googleByDayPercentGroup","fbByDayPercentGroup"], function(md,container,dc,d3,dateDim,googleByDayPercentGroup,fbByDayPercentGroup)
{
  ...
  compositeChart
              ...
              .compose([
                  dc.lineChart(compositeChart)
                    .group(googleByDayPercentGroup, 'Google')
                    .ordinalColors(['steelblue']),
                  dc.lineChart(compositeChart)
                    .group(fbByDayPercentGroup, 'Facebook') 
                    .ordinalColors(['darkorange'])
              ])
  dc.renderAll()
  return view
  ...
  main.variable(observer("moviesDataset")).define("moviesDataset", ["d3"], function(d3){return(
d3
...
md`## Exercício 2`
  ...
  main.variable(observer("movies")).define("movies", ["crossfilter","moviesDataset"], function(crossfilter,moviesDataset){return(
crossfilter(moviesDataset)
)});
  main.variable(observer("yearsDim")).define("yearsDim", ["movies"], function(movies){return(
movies.dimension(d => d.year)
)});
  main.variable(observer()).define(["yearsDim"], function(yearsDim){return(
yearsDim.top(10)
)});
  main.variable(observer("worldwideGrossPerYearGroup")).define("worldwideGrossPerYearGroup", ["yearsDim"], function(yearsDim){return(
yearsDim.group().reduceSum(d => d.gross_m)
)});
  main.variable(observer("by_year")).define("by_year", ["md","container","dc","d3","yearsDim","worldwideGrossPerYearGroup"], function(md,container,dc,d3,yearsDim,worldwideGrossPerYearGroup)
{
  ...
  let xScale = d3.scaleLinear()
                  .domain([yearsDim.bottom(1)[0].year - 0.5, yearsDim.top(1)[0].year + 0.5])
  ...
  barChart.width(700)
           ...
           .x(xScale)
           .dimension(yearsDim)
           .group(worldwideGrossPerYearGroup, 'Arrecadado')
  barChart.render()
  return view 
}
);
...
movies.dimension(d => d.genre)
  ...
  main.variable(observer("worldwideGrossPerGenreGroup")).define("worldwideGrossPerGenreGroup", ["genreDim"], function(genreDim){return(
genreDim.group().reduceSum(d => d.gross_m).order(d => d["Worldwide_Gross_M"])
)});
  main.variable(observer("by_genre")).define("by_genre", ["md","container","dc","d3","genreDim","worldwideGrossPerGenreGroup"], function(md,container,dc,d3,genreDim,worldwideGrossPerGenreGroup)
{
  ...
  barChart.width(700)
           ...
           .dimension(genreDim)
           .group(worldwideGrossPerGenreGroup)
  barChart.render()
  return view 
}
);
