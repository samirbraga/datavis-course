...
d3.schemeOranges[7]
  ...
  main.variable(observer("colorScale")).define("colorScale", ["d3","dataset","oranges"], function(d3,dataset,oranges){return(
d3.scaleQuantize()
...
function zoomToFeature(e) {
  ...
  main.variable(observer("info")).define("info", ["L","dataByName","map"], function(L,dataByName,map)
{
  // control that shows state info on hover
	...
	infoControl.update = function (feat) {
			this._div.innerHTML = '<h5>Número de homicídios</h5>' +  (feat ?
				'<b>' + feat.properties.NOME + '</b><br /> Taxa de homicídio: ' + dataByName.get(feat.properties.NOME).rate
...
d3.csv("https://gist.githubusercontent.com/emanueles/8ee225792454a78151d0842a10642a29/raw/9984e32effa99da3cea2ec87a7c9b7b231c71d3c/fortaleza_crimes_populacao.csv")
  ...
  main.variable(observer("dataByName")).define("dataByName", ["d3"], function(d3){return(
d3.csv("https://gist.githubusercontent.com/emanueles/8ee225792454a78151d0842a10642a29/raw/9984e32effa99da3cea2ec87a7c9b7b231c71d3c/fortaleza_crimes_populacao.csv").then(function(data) {
    ...
    data.forEach(function(d) {
       bairrosMap.set(d.Bairro, {
         homicides: +d.Homicidios,
         population: +d.Populacao,
         rate: d.Populacao === -1 ? 0 : (d.Homicidios * 100000) / d.Populacao
       })
    })
    return bairrosMap
  })
)});
  main.variable(observer("style")).define("style", ["dataByName","colorScale"], function(dataByName,colorScale){return(
function style(feature) {
  const data = dataByName.get(feature.properties.NOME);
  return {
    weight: 1,
    opacity: 1,
    color: 'white',
    dashArray: '3',
    fillOpacity: 0.6,
    fillColor: colorScale(data.rate)
  };
}
)});
