...
md`# Taxa de desemprego nos EUA em agosto de 2016`
  ...
  main.variable(observer("buildvis")).define("buildvis", ["d3","DOM","rateById","topojson","us"], function(d3,DOM,rateById,topojson,us)
{
  const width = 960
  const height = 600
  const svg = d3.select(DOM.svg(width, height))
  let path = d3.geoPath()
  const cGreens = d3.scaleSequential(d3.interpolateGreens).domain([1, 10]);
  const greenById = id => cGreens(rateById.get(id))
